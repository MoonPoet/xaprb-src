...
The good news is, there's a way to get around these {{< math >}}n^2{{< /math >}} performance penalties! Since *I* know the first row it finds is the one it wants, even though *it* doesn't know that, I can tell it to scan the clustered index and stop after one row. Here's the query:
