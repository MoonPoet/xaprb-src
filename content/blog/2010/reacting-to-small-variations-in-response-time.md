...
1.  There are normally very few connections to the server, but sometimes I start getting "Error: `max_connections` exceeded" or similar. *Solution: observe `Threads_connected` from SHOW GLOBAL STATUS and react when it grows too large, or when you cannot log in to query `Threads_connected`.*
